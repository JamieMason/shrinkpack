...
const testPackageJson = path.join(projectRoot, 'testing/package.json');
// const sinopiaConfigPath = path.join(projectRoot, 'testing/sinopia/config.yaml');
// let killSinopia;
let startingNpmVersion;

test.before(async () => {
  startingNpmVersion = await testUtil.getNpmVersion();
  // killSinopia = await runSinopia(sinopiaConfigPath);
